...
def centralized_sgd(setting, attack=None, save_model=False, save_results=False):
    ...
    # epsilon:privacy budget gamma:flip prob
    args = args_parser()
    exp_details(args)
    lambda0 = 0.01
    u = 0.004
        ...
        if (iteration == 1) or (iteration % 50 == 0):
            ...
            for id in regular:
                ...
                for index, (para, para0) in enumerate(zip(model[id].parameters(), model0.parameters())):
                    ...
                    # flip the sign gradient(CreFlip)
                    # c : unbiased correction
                    random_flip = torch.rand_like(para)
                    flip = torch.ones_like(para)
                    flip[random_flip > gamma] = -1
                    c = (np.exp(epsilon) + 1) / (np.exp(epsilon) - 1)
                    signinfo[id][index].data.add_(torch.sign(para0.data - para.data) * flip, alpha=c)
    ...
    if args.attack == sign_flipping:
        file_name = 'sign_flipping/CreFlip_user[{}]_byzanusers[{}]_duli{}_lr[{}]_eps[{}].pkl'. \
            format(args.num_users, args.byzantinue_users, args.iid, args.lr,
                   args.eps)
        file_name_cre = 'sign_flipping/CreFlip_xinyu_user[{}]_byzanusers[{}]_duli{}_lr[{}]_eps[{}].pkl'. \
            format(args.num_users, args.byzantinue_users, args.iid, args.lr,
                   args.eps)
    elif args.attack == same_value:
        file_name = 'same_value/CreFlip_user[{}]_byzanusers[{}]_duli{}_lr[{}]_eps[{}].pkl'. \
            format(args.num_users, args.byzantinue_users, args.iid, args.lr,
                   args.eps)
        file_name_cre = 'same_value/CreFlip_xinyu_user[{}]_byzanusers[{}]_duli{}_lr[{}]_eps[{}].pkl'. \
            format(args.num_users, args.byzantinue_users, args.iid, args.lr,
                   args.eps)
    elif args.attack == gauss_attack:
        file_name = 'gauss_attack/CreFlip_user[{}]_byzanusers[{}]_duli{}_lr[{}]_eps[{}].pkl'. \
            format(args.num_users, args.byzantinue_users, args.iid, args.lr,
                   args.eps)
        file_name_cre = 'gauss_attack/CreFlip_xinyu_user[{}]_byzanusers[{}]_duli{}_lr[{}]_eps[{}].pkl'. \
            format(args.num_users, args.byzantinue_users, args.iid, args.lr,
                   args.eps)
    elif args.attack == zero_gradient:
        file_name = 'zero_gradient/CreFlip_user[{}]_byzanusers[{}]_duli{}_lr[{}]_eps[{}].pkl'. \
            format(args.num_users, args.byzantinue_users, args.iid, args.lr,
                   args.eps)
        file_name_cre = 'zero_gradient/CreFlip_xinyu_corrflip_user[{}]_byzanusers[{}]_duli{}_lr[{}]_eps[{}].pkl'. \
            format(args.num_users, args.byzantinue_users, args.iid, args.lr,
                   args.eps)
    elif args.attack == sample_duplicating:
        file_name = 'sample_duplicating/CreFlip_user[{}]_byzanusers[{}]_duli{}_lr[{}]_eps[{}].pkl'. \
            format(args.num_users, args.byzantinue_users, args.iid, args.lr,
                   args.eps)
        file_name_cre = 'sample_duplicating/CreFlip_xinyu_user[{}]_byzanusers[{}]_duli{}_lr[{}]_eps[{}].pkl'. \
            format(args.num_users, args.byzantinue_users, args.iid, args.lr,
                   args.eps)
